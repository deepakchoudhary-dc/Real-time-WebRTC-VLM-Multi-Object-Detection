...
function validateIceUrl(url, validSchemes = ['stun:', 'turn:', 'turns:']) {
  if (!url || typeof url !== 'string') return null;
  const trimmed = url.trim();
  const hasValidScheme = validSchemes.some((s) => trimmed.toLowerCase().startsWith(s));
  return hasValidScheme ? trimmed : null;
}

const rawStunUrls = process.env.STUN_URLS
  ? process.env.STUN_URLS.split(',').map((u) => validateIceUrl(u, ['stun:'])).filter(Boolean)
