...
async function processFrame(connection, frame) {
  ...
  try {
    ...
    if (MODE === 'server') {
      detections = await processFrameServer(frame);
    } else {
      // In WASM mode, signal client to process
    ...
    connection.socket.emit('detection-result', result);
    
    // Update metrics
    connection.stats.framesProcessed++;
    metrics.processedFrames++;
    
    const latency = inference_ts - frame.capture_ts;
    metrics.latencies.push(latency);
    connection.stats.avgLatency = 
      (connection.stats.avgLatency * (connection.stats.framesProcessed - 1) + latency) 
      / connection.stats.framesProcessed;
...
// Server-side inference (COMPLETELY DISABLED - NO FAKE DATA)
async function processFrameServer(frame) {
  // NEVER return fake detections
  console.log('🚫 Server-side detection disabled - no fake data will be sent');
  return []; // Always empty - no fake bicycle/cat/dog
}
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`🌐 Open http://localhost:${PORT} to start (desktop)`);
  console.log(`📱 Phone: Access http://${localIP}:${PORT}/phone`);
